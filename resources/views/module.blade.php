<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	  <meta name="viewport" content="width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=1">
        <title>Disponobilite</title>
		<link rel="stylesheet" href="{{ asset('css/bootstrap.min.css') }}">
        <link rel="stylesheet" href="{{ asset('css/custom.css') }}">
        
		
		<!--google fonts -->
		<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" integrity="sha512-SnH5WK+bZxgPHs44uWIX+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkcmfRyVX3pBnMFcV7oQPJkl9QevSCWr3W6A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
	    <link rel="preconnect" href="https://fonts.googleapis.com">
        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
        <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">
	
	
	   <!--google material icon-->
      <link href="https://fonts.googleapis.com/css2?family=Material+Icons"rel="stylesheet">
      
	  
	  <link rel="stylesheet" href="{{ asset('css/modul.css') }}">

  </head>
  <body>
  


<div class="wrapper">
     
	  <div class="body-overlay"></div>
	 
	 <!-------sidebar--design------------>
	 
	 <div id="sidebar">
	    <div class="sidebar-header">
		   <!-- <h3><img style="padding-top: 10px; border-radius: 10%;" src="https://img.freepik.com/premium-photo/businessman-portrait-ai-generated-image_268835-5686.jpg" class="img-fluid"/>
			<span>Mr.hourri<br></span>
			<h6 style="padding-left:60px;size: 10px !important;">Admin</h6> -->
		<h3>	
			<img  src="data:image/png;base64," style="border-radius: 25px;" class="img-fluid"/>
			<span>EST SAFI</span>
		</h3>
		
    </div>
    <ul class="list-unstyled component m-2">
      <li class="active">
      <!-- <a href="#" class="dashboard"><span <i class="fa-solid fa-house"></i>&nbsp;&nbsp;&nbsp;
        </span>Home </a>
      </li> -->
      
      <li class="">
        <a href="{{ route('home') }}" class=""><span  <i  class="fa-solid fa-house"></i>&nbsp;&nbsp;&nbsp;
        </span>Accueil</a>
        </li>
      <li class="">
            <a href="{{ route('profile') }}"  class=""><span <i class="fa-solid fa-book"></i>&nbsp;&nbsp;&nbsp;
            </span>Profile </a>
            </li>

      <li class="">
      <a href="{{ route('dispo') }}" class=""><span <i class="fa-solid fa-school"></i>&nbsp;&nbsp;&nbsp;
      </span>Disponibilite </a>
      </li>
      <li class="">
        <a href="{{ route('module') }}" class=""><span <i class="fa-solid fa-book"></i>&nbsp;&nbsp;&nbsp;
        </span>Module</a>
        </li>
        <li class="">
            <a href="{{ route('emploi') }}" class=""><span <i class="fa-solid fa-book"></i>&nbsp;&nbsp;&nbsp;
            </span>Emploi du temps</a>
            </li>
            
                <li class="">
                    <a href="{{ route('login') }}" class=""><span <i class="fa-solid fa-right-from-bracket"></i>&nbsp;&nbsp;&nbsp;
                    </span>Se deconnercter</a>
                    </li>
     
    
    </ul>
 </div>
	 
   <!-------sidebar--design- close----------->
   
   
   
      <!-------page-content start----------->
   
      <div id="content">
	     
		  <!------top-navbar-start-----------> 
		     
		  <div class="top-navbar">
		     <div class="xd-topbar">
			     <div class="row">
				     <div class="col-2 col-md-1 col-lg-1 order-2 order-md-1 align-self-center">
					    <div class="xp-menubar">
						    <span class="material-icons text-white">signal_cellular_alt</span>
						</div>
					 </div>
					 
					 <div class="col-md-5 col-lg-3 order-3 order-md-2">
					     <div class="xp-searchbar">
						     <form>
							    <div class="input-group">
								  <input type="search" class="form-control"
								  placeholder="Search">
								  <div class="input-group-append">
								     <button class="btn" type="submit" id="button-addon2">Go
									 </button>
								  </div>
								</div>
								<script>
									document.querySelector('.btn').addEventListener('click', function() {
                                     var inputValue = document.querySelector('.form-control').value;
                                  // Implement search functionality here
                                     console.log('Searching for:', inputValue);
                                       });
								</script>
							 </form>
						 </div>
					 </div>
					 
					 
					 <div class="col-10 col-md-6 col-lg-8 order-1 order-md-3">
					     <div class="xp-profilebar text-right">
						    <nav class="navbar p-0">
							   <ul class="nav navbar-nav flex-row ml-auto">
							   <li class="dropdown nav-item active">
							     <a class="nav-link" href="#" data-toggle="dropdown">
								  <span class="material-icons">notifications</span>
								  <span class="notification">4</span>
								 </a>
								  <ul class="dropdown-menu">
								     <li><a href="#">You Have 4 New Messages</a></li>
									 <li><a href="#">You Have 4 New Messages</a></li>
									 <li><a href="#">You Have 4 New Messages</a></li>
									 <li><a href="#">You Have 4 New Messages</a></li>
								  </ul>
							   </li>
							   
							   <li class="nav-item">
							     <a class="nav-link" href="#">
								   <span class="material-icons">question_answer</span>
								 </a>
							   </li>
							   
							   <li class="dropdown nav-item">
							     <a class="nav-link" href="#" data-toggle="dropdown">
									<img id="profileimage" src="{{ Auth::user()->profileimage ? asset('storage/' . Auth::user()->profileimage) : asset('assets/icon.jpg') }}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 50%;">
									<span class="xp-user-live"></span>
								   </a>
									<ul class="dropdown-menu small-menu">
									   <li><a href="index.html">
									   <span class="material-icons">person_outline</span>
									   Profile
									   </a></li>
									   <li><a href="#">
									   <span class="material-icons">settings</span>
									   Settings
									   </a></li>
									   
									   <li><a href="signup.html">
									   <span class="material-icons">logout</span>
									   Logout
									 </a></li>
									 
								  </ul>
							   </li>
							   
							   
							   </ul>
							</nav>
						 </div>
					 </div>
					 
				 </div>
				 
				
				 
			 </div>
		  </div>

		  
        
		  <div class="container">
			<div class="col-md-3">
				<label>Choisir votre semestre</label>
				<select class="form-control" id="cycle">
					<option value="1">semestre 1</option>
					<option value="2">semestre 2</option>
					<option value="3">semestre 3</option>
					<option value="4">semestre 4</option>
				</select>
			</div>
			<div class="col-md-3">
				<label>Choisir votre module</label>
				<select class="form-control" id="module">
					<option value="1">Reseau</option>
					<option value="2">Administration reseau</option>
					<option value="3">Systeme d'exploitation</option>
				</select>
			</div>
            <div class="form-group">
				<label for="nombreCours">Nombre de groupes de cours à choisir :</label>
				<input type="number" id="nombreCours" name="nombreCours" min="0" value="0">
			</div>
			<div class="form-group">
				<label for="nombreTD">Nombre de groupes de TD à choisir :</label>
				<input type="number" id="nombreTD" name="nombreTD" min="0" value="0">
			</div>
			<div class="form-group">
				<label for="nombreTP">Nombre de groupes de TP à choisir :</label>
				<input type="number" id="nombreTP" name="nombreTP" min="0" value="0">
			</div>
		
			<div class="form-group">
				<label for="groupeCoursDispo">Choisir les groupes de cours que vous voulez</label>
				<div class="form-check">
					<input class="form-check-input groupeCours" type="checkbox" id="groupeCours1" name="groupeCoursDispo" value="1">
					<label class="form-check-label" for="groupeCours1">Groupe 1</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeCours" type="checkbox" id="groupeCours2" name="groupeCoursDispo" value="2">
					<label class="form-check-label" for="groupeCours2">Groupe 2</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeCours" type="checkbox" id="groupeCours3" name="groupeCoursDispo" value="3">
					<label class="form-check-label" for="groupeCours3">Groupe 3</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeCours" type="checkbox" id="groupeCours4" name="groupeCoursDispo" value="4">
					<label class="form-check-label" for="groupeCours4">Groupe 4</label>
				</div>
			</div>
		
			<div class="form-group">
				<label for="groupeTDDispo">Choisir les groupes de TD que vous voulez</label>
				<div class="form-check">
					<input class="form-check-input groupeTD" type="checkbox" id="groupeTD1" name="groupeTDDispo" value="1">
					<label class="form-check-label" for="groupeTD1">Groupe 1</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTD" type="checkbox" id="groupeTD2" name="groupeTDDispo" value="2">
					<label class="form-check-label" for="groupeTD2">Groupe 2</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTD" type="checkbox" id="groupeTD3" name="groupeTDDispo" value="3">
					<label class="form-check-label" for="groupeTD3">Groupe 3</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTD" type="checkbox" id="groupeTD4" name="groupeTDDispo" value="4">
					<label class="form-check-label" for="groupeTD4">Groupe 4</label>
				</div>
			</div>
		
			<div class="form-group">
				<label for="groupeTPDispo">Choisir les groupes de TP que vous voulez</label>
				<div class="form-check">
					<input class="form-check-input groupeTP" type="checkbox" id="groupeTP1" name="groupeTPDispo" value="1">
					<label class="form-check-label" for="groupeTP1">Groupe 1</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTP" type="checkbox" id="groupeTP2" name="groupeTPDispo" value="2">
					<label class="form-check-label" for="groupeTP2">Groupe 2</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTP" type="checkbox" id="groupeTP3" name="groupeTPDispo" value="3">
					<label class="form-check-label" for="groupeTP3">Groupe 3</label>
				</div>
				<div class="form-check">
					<input class="form-check-input groupeTP" type="checkbox" id="groupeTP4" name="groupeTPDispo" value="4">
					<label class="form-check-label" for="groupeTP4">Groupe 4</label>
				</div>
			</div>
		
			<div class="form-group">
				<p id="groupesCoursRestants"></p>
				<p id="groupesTDRestants"></p>
				<p id="groupesTPRestants"></p>
			</div>
		
			<div class="form-group">
				<button type="button" class="btn btn-primary" onclick="addInformation()">Ajouter</button>
				<button type="button" class="btn btn-secondary" onclick="modifyInformation()">Modifier</button>
				
			</div>
		
			<script>
				function calculerGroupesRestants() {
					let nombreCours = parseInt(document.getElementById('nombreCours').value) || 0;
					let nombreTD = parseInt(document.getElementById('nombreTD').value) || 0;
					let nombreTP = parseInt(document.getElementById('nombreTP').value) || 0;
		
					let groupeCours = document.querySelectorAll('.groupeCours:checked').length;
					let groupeTD = document.querySelectorAll('.groupeTD:checked').length;
					let groupeTP = document.querySelectorAll('.groupeTP:checked').length;
		
					let groupesCoursRestants = nombreCours - groupeCours;
					let groupesTDRestants = nombreTD - groupeTD;
					let groupesTPRestants = nombreTP - groupeTP;
		
					document.getElementById('groupesCoursRestants').innerText = Groupes de cours restants : ${groupesCoursRestants};
					document.getElementById('groupesTDRestants').innerText = Groupes de TD restants : ${groupesTDRestants};
					document.getElementById('groupesTPRestants').innerText = Groupes de TP restants : ${groupesTPRestants};
				}
		
				function addInformation() {
					// Function to add information and calculate remaining groups
					calculerGroupesRestants();
					alert("Informations ajoutées et groupes restants calculés.");
				}
		
				function modifyInformation() {
					// Function to modify information and recalculate remaining groups
					calculerGroupesRestants();
					alert("Informations modifiées et groupes restants recalculés.");
				}
		
				
			</script>
                  <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
                  <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
                  <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
                  <script src="{{ asset('js/jquery-3.3.1.slim.min.js') }}"></script>
                  <script src="{{ asset('js/popper.min.js') }}"></script>
                  <script src="{{ asset('js/bootstrap.min.js') }}"></script>
                  <script src="{{ asset('js/jquery-3.3.1.min.js') }}"></script>
                  <script src="{{ asset('js/list.js') }}"></script>
<script type="text/javascript">
$(document).ready(function(){
$(".xp-menubar").on('click',function(){
  $("#sidebar").toggleClass('active');
  $("#content").toggleClass('active');
});

$('.xp-menubar,.body-overlay').on('click',function(){
   $("#sidebar,.body-overlay").toggleClass('show-nav');
});

});
</script>   
		</body>
		</html>
